...
NW_CHECK () {
	...
	INC=$(ifconfig -a | egrep "bond|eth" |awk '{print $1}')
	
		for i in $INC
			do
		IPA=$(ifconfig $i | grep inet 2> /dev/null |awk '{print $2}' | awk -F: '{print $2}')
			if [ $(ifconfig $i | grep inet 2> /dev/null |awk '{print $2}' | awk -F: '{print $2}' | wc -l) -ne 0 ]; then
			IPS="$IPS;$IPA;$i"$'\n'
			fi
			done
