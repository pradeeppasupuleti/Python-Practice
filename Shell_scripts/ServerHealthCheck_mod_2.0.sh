...
if [ $($CAT /etc/redhat-release | $AWK -F "release" '{print $2}' | $AWK -F "." '{print "RHEL-"$1}' | sed 's/ //g' | $AWK -F "-" '{print $2}' | cut -c 1) -le 3 ]; then

	VG=$(vgdisplay | grep "VG Name" | $AWK '{print $3}' | wc -l)
	LV=$(lvscan | $AWK -F "\"" '{print $2}' | grep -v "^$" | wc -l)

	for i in `lvscan | $AWK -F "\"" '{print $2}' | grep -v "^$"`
		do
