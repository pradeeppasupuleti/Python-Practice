...
def get_bdcs_clusters(uri=''):
    url="https://infrainternal.us2.cloud.oracle.com:7777/SDI-WS/SDIControlService?wsdl"
    client = Client(url)
...
# Function to return dc_code using sdictl.
...
def query_sm(cu='', cp='', url='', idomain=''):
    try:
        ...
        cp="bdc$_App1d"
        url="https://paassvcmngrinternal-us2-sm02.oraclecloud.com:8888/paas/bdcs/api/v1.0/services"
...
def resource_table(dc_code, resource_dict):
    ...
    out.writerow(['America Resource Statistics',''])
...
def active_customers_table(dc_code, active_customers_data):
    ...
    table_data += '<th>Subnet</th><th>Cluster Name</th><th>Cluster Status</th><th>Created by</th><th>Datacenter</th>'
    ...
    out = csv.writer(resultFile, delimiter=',',quoting=csv.QUOTE_ALL)
    out.writerow(['America customers and segment allocation', ''])
...
def deleted_customers_table(deleted_customers_data):
    ...
    table_data += '<table border=1 cellpadding=1 cellspacing=1 style="border-collapse: collapse;">'
    table_data += '<tr bgcolor="#b5a434"><th>Customer</th><th>Customer status</th><th>Identity Domain</th><th>Service Name</th><th>Createdby</th><th>Datacenter</th>'
